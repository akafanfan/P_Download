...
def export_subscriptions():
    ...
    logging.info("Exported %i subscriptions. Create file at %s", len(exported_subscriptions),
...
def get_subscription_data_obj(url: str, downloaded=None, last_checked=None, last_metadata=None):
    ...
    if not metadata:
        ...
        return subscription_entry
    logger.info("当前订阅作者的视频总数%s", metadata.get('playlist_count'))
    ...
    entry_in_db = fetch_value("subscriptions",
                              ...
                              ["id", "scheme", "subscription_name"], True)
    # entry_in_db = (1, 'pornhub', 'comatozze')
...
def direct_download(url: str, title: str, own_file_data: dict = None):
...
def download_file(url, path, metadata=None, ignore_existing_url=False):
    ...
    if metadata is None:
        logging.error(
            "Error while fetching metadata to check if video already exists in db! - Continue without checking")
        return return_val
...
def download_missing():
    ...
    subscriptions = fetch_value("subscriptions", None,
                                ["scheme",
                                 ...
                                 "current_subscription_data",
                                 "subscription_last_checked"], None, "ORDER BY scheme")
    ...
    for current_dir, current_dir_directories, current_dir_files in os.walk(base_path):
        for file in current_dir_files:
            ...
            if path_data["schema_name"] is None or path_data["schema_name"] == "custom":
                logger.warning("File %s can not be tested! - Schema \"%s\" is not valid", path_data["filename"],
                               path_data["schema_name"])
                continue

            # Check if Video is in DB
            file_in_db = fetch_value("items", {"file_name": path_data["filename"]},
                                     ["file_name", "scheme", "file_hash", "locked"], True)
            ...
            if file_in_db is None:
                ...
                if not file_saved["status"]:
                    ...
                    add_duplicate_file(file_hash["hash"], path_data["filename"], os.path.abspath(current_dir),
                                       file_saved["file_id"], file_saved["file_name"], file_saved["file_path"])
                else:
...
def get_expected_filepath(metadata: dict, path: str, title: str):
    """
    ...
    filename = YoutubeDL(get_ydl_opts(path)).prepare_filename(metadata, outtmpl=path + '/' + title + '%(ext)s')
...
def check_is_url_in_items_db(url, filename=None, file_path=None, filename_is_id=False):
    ...
    if filename is None and file_path is None:
        ...
        if not filename_is_id:
            ...
            fetched_urls = fetch_value("items", {"file_name": filename, "file_path": file_path},
                                       ["id", "file_name", "file_path", "url"])
        else:
        ...
        if hash_value in duplicates_json:
            ...
            for entry in listing:
                if not os.path.isfile(os.path.abspath(os.path.join(entry["file_path"], entry["file_name"]))):
                    ...
                    new_listing.append(
                        {"file_id": entry["file_id"], "file_name": entry["file_name"], "file_path": entry["file_path"]})
